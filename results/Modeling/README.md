...
- **다중 지표를 일괄 비교**  
  단일 지표가 모든 의사결정 포인트를 설명하기 어렵기 때문에, `MetaScore = 0.5*F2 + 0.3*PR-AUC + 0.2*ROC-AUC`로 정의했습니다(`results/parameter_tuning/meta_score.py`). F2로 재현율 우선 전략을 유지하면서도, PR-AUC(불균형 정밀도-재현율 균형)와 ROC-AUC(전반적 분류력)를 반영해 모델 간 우열을 한 번에 비교할 수 있습니다.
